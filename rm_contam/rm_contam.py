...
class Bowtie2Aligner:
    ...
    def single_reads(self, file, processed_folder, samtools_folder):
        ...
        """
        try:
            sam_output = samtools_folder/f"{file.name}_mapped.sam"
            rmcontam_output = processed_folder/f"{file.name}_unmapped.fastq.gz"
            contam_output = processed_folder/f"{file.name}_mapped.fastq.gz"
            cmd = ["bowtie2", 
